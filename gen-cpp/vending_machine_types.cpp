...
const std::map<int, const char*> _Beverages_VALUES_TO_NAMES(::apache::thrift::TEnumIterator(6, _kBeveragesValues, _kBeveragesNames), ::apache::thrift::TEnumIterator(-1, NULL, NULL));

std::ostream& operator<<(std::ostream& out, const Beverages::type& val) {
  std::map<int, const char*>::const_iterator it = _Beverages_VALUES_TO_NAMES.find(val);
  if (it != _Beverages_VALUES_TO_NAMES.end()) {
    out << it->second;
  } else {
    out << static_cast<int>(val);
  }
...
std::string to_string(const Beverages::type& val) {
  std::map<int, const char*>::const_iterator it = _Beverages_VALUES_TO_NAMES.find(val);
  if (it != _Beverages_VALUES_TO_NAMES.end()) {
    return std::string(it->second);
  } else {
